...
import {ITreasury, IStakERC20, IRootNft, ISwapper, IP2PLending} from "./interfaces.sol";
...
modifier validateAction(uint _proposalId) {
    ...
    modifier hasMinimumRootBalance(address _member) {
        require(IERC721(RootNft).balanceOf(_member) > 0, "not qualified");
        require(
            IERC20(RootErc20).balanceOf(_member) >= (100 ether),
            "not qualified"
        );
        _;
    }
   ...
   function usernameExists(string memory _username) public view returns (bool) {
    ...
    ) external hasMinimumRootBalance(_member) {
    ...
    function mintNft(string memory _uri) public {
        uint stakedbalance = IStakERC20(stakeErc20)
            .stakeassets(msg.sender, RootErc20)
            .capital;
        require(stakedbalance >= 1000 ether, "not qualified");
        IRootNft(RootNft).safeMint(msg.sender, _uri);
    }
