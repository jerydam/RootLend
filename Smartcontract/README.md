...
Powered by the Rootstock test network, RootLend brings together robust smart contracts, seamless integrations, and a highly intuitive front-end interface. Users enjoy complete control over their assets and data, secured through wallet or email login — ensuring privacy and security at every step.

With the RootLend dashboard, accessing the platform's full suite of features is effortless. Unlike traditional financial systems that involve lengthy approvals, high fees, and intermediaries, RootLend provides:
...
* Lower fees and better rates thanks to the low gas fees on Rootstock network and the elimination of middlemen
* 24/7 global accessibility — no waiting for business hours or paperwork
* Enhanced security and user ownership of funds

## Benefits for Lenders on RootLend:
...
Lenders can communicate directly with borrowers through RootLend’s decentralized chat while maintaining full privacy.

### Participation in a Growing DeFi Ecosystem:
As RootLend grows and adds more supported tokens and features, lenders benefit from a wider range of investment opportunities.

## Benefits for Borrowers on RootLend:
...
Frequent successful repayments can help borrowers build a reputation within the RootLend ecosystem, potentially making it easier to get better loan terms in the future.
RootLend delivers a lending experience that not only matches but exceeds the capabilities of traditional finance, empowering users with true financial freedom.

## Features of RootLend
...
### Web3 Integrations
Web3Modal, Ethers.js, Rootstock Testnet Token Tracker
...
## The RootLend User Flow

### Connect Wallet
![File](https://supposed-emerald-snake.myfilebase.com/ipfs/QmSNNevtfaJKNqd8Umbd7cUktwvoRx4wdNeRPdqKGrejQF)
To get started, visit our [Web App](https://rootlend.vercel.app).
...
#### On Mobile:
Download an EVM-compatible wallet (such as MetaMask or Trust Wallet) and use its built-in browser to access RootLend. Then, click "Connect Wallet" at the top right.

After initiating the connection, ensure you switch the network to the Rootstock Test Network via the wallet connect interface.
Once connected, your dashboard display will update — typically showing your Rootstock RBTC token balance along with a portion of your wallet address.

### Get testnet Rootstock RBTC for gas fees payments
![File](https://ipfs.filebase.io/ipfs/QmeTAgrea7RSrNT3siQuFW98TBsx1SVxpVEemYCwE6UEs4)
Since RootLend is currently deployed on the Rootstock network's testnet, you will need testnet RBTC to cover gas fees during transactions.
You can easily obtain testnet RBTC through the [Rootstock testnet faucet](https://faucet.rootstock.io/).

### Web app homepage
The homepage of the RootLend web app is the first page you’ll see when you visit the platform. It serves as the gateway to the RootLend dashboard and features the Connect Wallet button, along with important resources such as platform information and documentation.

 ### The RootLend Dashboard
This is the core functional area of RootLend, bringing together the platform’s most essential features — including user balances, key metrics, the P2P lending and borrowing dApp and Swap.

### User's Balances
![File](https://supposed-emerald-snake.myfilebase.com/ipfs/QmfW4RJyU9qRiCxATavqmsFa32um7FeyDLkHEuvh5RtHeF)
This section of the dashboard shows all the balances of a user after logging in. It displays the following info for a user: RTL balance, RTL price, RBTC balance, number of loans you created, number of loans you funded.

### Metrics
![File](https://supposed-emerald-snake.myfilebase.com/ipfs/Qmaw6nob8UK24Yc5oRazrmyahSLD3Y15tzwh2dCkKNyGR1)
The metrics section shows live updates to RootLend including the following: RTL Total Supply, RTL Price, Protocol Blockchain, Total number of Loans, Max Loan Amount, Min Loan Amount, contract addresses of all contracts used by the protocol and Live RBTC market chart.


## P2P Lend/Borrow dApp
RootLend enables users to participate in unique P2P lending and borrowing activities involving supported tokens. To initiate the process, a borrower creates a loan request using the "Approve/Create Loan" buttons in the Borrow section. A lender can then fund the loan by using the "Fund Loan" button in the Lend section.
Each loan on RootLend is defined by a loan amount, an interest rate, an expiry date, and collateral. The borrower's collateral is securely locked by the system until the loan's expiry. If no lender funds the loan before the expiry, the borrower can reclaim their collateral using the "Withdraw Collateral" button in the Lend section. Borrowers who successfully receive funding can withdraw the borrowed tokens using the "Withdraw Loan" button.
If a borrower fails to repay the loan by the expiry date, the lender has the right to claim the posted collateral through the "Claim Collateral" button. Lenders are rewarded generously based on the agreed terms of the loan, with additional security provided by RootLend’s advanced security algorithms.
When borrowing through RootLend, users must present sufficient collateral and repay the borrowed amount with interest using the "Repay Loan" button to fulfill their obligations.
...
New supported tokens and collateral options will continue to be added to the platform over time, ensuring RootLend remains dynamic and versatile.

### Borrowing
![File](https://supposed-emerald-snake.myfilebase.com/ipfs/QmaJc5aSwKB287ih5AZzxq1GoVsPwee32GokcnfyJBgf8u)
To borrow, follow the steps below (Your wallet will popup to sign the transactions):
* Fill the details above setting the duration, amount of RBTC to borrow, collateral, collateral amount, "yes if collateral is ERC20 or no if collateral is ERC721 (an NFT)", and interest rate (which is the amount of interest you will pay to any user that funds your loan).
...
### Lending
![File](https://supposed-emerald-snake.myfilebase.com/ipfs/QmUZVWAmPwPN9uycffmPvkw1y5Gktxgh3ag9zBRMrXKTWE)

![File](https://supposed-emerald-snake.myfilebase.com/ipfs/QmZ3zeCg7KsWGoQ1cD3LsbRUCaojbmw8ZNta2wrmqigsBY)
Lending on RootLend is simpler than you might expect. Thanks to a smooth and intuitive user experience, users can easily carry out all lending activities. The platform features dedicated sections like "View All Loans", "Loans You Funded", and "Loans You Created" to help users effortlessly track their loan activities.
...
### RootLend Decentralized Chat System
![File](https://supposed-emerald-snake.myfilebase.com/ipfs/QmVaMwhfTyFLAYDHNNyY9Uz83fzBA4Zic5qRL6GvUdgbmy)
For effective communication between borrowers and lenders, RootLend provides a built-in decentralized chat system. Only the borrower and lender associated with a specific loan can communicate through this chat.
...
### RootLend Token (RTL)
The RootLend Token (RTL) is the native token of the RootLend platform and is primarily used as collateral for lending activities.
In addition to RTL, users can also use other supported tokens like USDT and DAI as collateral.

## RootLend Swap Feature with ChainLink Oracles
![File](https://supposed-emerald-snake.myfilebase.com/ipfs/QmTdTiPYcx9vmYBrWjjnvDf4wZvfKHp6CiuGWpyVjCu3mi)
RootLend offers a seamless and secure token swap feature, integrated with ChainLink Oracles to ensure accurate and reliable price feeds. This allows users to efficiently swap a variety of tokens directly within the platform.
...
Whether you're a lender, borrower, or general user on RootLend, this swap feature provides flexibility and convenience, making it easy to exchange tokens directly within the platform. The process is fast, secure, and powered by decentralized oracles, ensuring that you get the best rates available at the time of your transaction.

## Additional Notes (Future Upgrades for RootLend)
In upcoming updates, RootLend will introduce a host of new features and improvements to enhance the user experience and expand the platform's capabilities:
...
Additional security measures will be implemented to further protect user assets and transactions, building on RootLend’s robust security architecture.

### Staking and Other Yield Farming Opportunities:
RootLend aims to introduce staking and other yield farming features, allowing users to earn passive income by staking their tokens or providing liquidity to the platform.

### Cross-Chain Compatibility:
We’re exploring the integration of cross-chain capabilities, allowing users to interact with RootLend using assets from other blockchain networks, enhancing accessibility and liquidity.

### Mobile App Development:
In the future, we plan to launch a mobile app, providing users with easy access to RootLend's features and functionality on the go.
...
Liquidity providers in the RootLend Swap will be rewarded with incentives. Users who supply liquidity to the swap pools (RBTC, RTL, USDT, DAI) will earn a share of the trading fees and possibly additional rewards in RTL tokens, encouraging a healthy liquidity environment.

## Team Members (Team RootLend)
* Patrick Ominisan - Blockchain developer
* Jeremiah Oyeniran - Smart contract developer


## Contract Addresses with Rootstock Testnet Links [Rootstock Testnet Explorer](https://explorer.testnet.rootstock.io)
![File](https://supposed-emerald-snake.myfilebase.com/ipfs/QmUHTHFTc8eKwcLP6tugvMwfAXp1gG6Rip31rbQFUap8g2)

**Note that all contracts are verified on the Rootstock testnet explorer.**

### P2P Lending/Borrowing Contract

✅ [P2PLending](https://explorer.testnet.rootstock.io/address/0x91f3ff344623adc499ec6a34fc6311e8abbf7880) 🟢
- Address: `0x91f3ff344623adc499ec6a34fc6311e8abbf7880`

### RootLend ERC20 (RTL) Contract

✅ [RootLend Token](https://explorer.testnet.rootstock.io/address/0x326e042259c7cf517ca303f6cbe732d33331645e) 🟢
- Address: `0x326e042259c7cf517ca303f6cbe732d33331645e`

### Swap Contract

✅ [Swap](https://explorer.testnet.rootstock.io/address/0x4df364ca65d0ae96cde5acf255ed5bd873af224b) 🟢
- Address: `0x4df364ca65d0ae96cde5acf255ed5bd873af224b`

## Other Tokens

### USDT

- **[USDT](https://explorer.testnet.rootstock.io/address/0xa4a6888685faeca07b5e42e00a05f92ad3b921ca)**: 0xa4a6888685faeca07b5e42e00a05f92ad3b921ca

### DAI

- **[DAI](https://explorer.testnet.rootstock.io/address/0xe2608d2d22e59ebf715114e976bdc8aa85891c1b)**: 0xe2608d2d22e59ebf715114e976bdc8aa85891c1b
...
- **Adds Initial Collaterals**: Adds `RTL`, `usdt`, and `dai` as accepted collaterals.
...
- **Deadline Check**: Verifies that the current block timestamp is within the loan's funding deadline.
- **Self-Funding Restriction**: Prevents borrowers from funding their own loans.
...
- **AI-Powered Trigger**: This function is typically triggered by a backend AI system (via a scheduled API route using Vercel Cron) that periodically evaluates and selects high-potential loans based on parameters such as interest rate and amount.
...
# RootLend Token Swap Feature with Chainlink Oracles

RootLend's token swap feature enables users to exchange tokens seamlessly within the platform, leveraging Chainlink Oracles for real-time, accurate price feeds. This ensures fair and transparent swap rates for supported tokens: RBTC, RTL (RootLend Token), USDT, and DAI. The feature is powered by a Solidity smart contract (`Swapper.sol`) that integrates liquidity pools, Chainlink price feeds, and secure token swapping mechanisms.


## Features
- **Supported Tokens:** Swap between RBTC, RTL, USDT, and DAI.
...
- **Flexibility:** Supports RBTC-to-token, token-to-RBTC, and token-to-token swaps.
...
- **token Prices:** Fallback prices for tokens (e.g., RBTC at $1755, RTL at 0.0003 RBTC).

### Swap Functions
- `swapTokensForTokens`: Swap between ERC20 tokens.
- `swapEthForTokens`: Swap RBTC for ERC20 tokens.
- `swapTokensForEth`: Swap ERC20 tokens for RBTC.

### Liquidity Functions
- `addLiquidity`: Add tokens or RBTC to a pool.
...
struct Token {
    ...
    address tokenAddress; // Address of the token (0x0 for RBTC)
    uint256 decimal;      // Number of decimals (18 for RBTC, USDT, DAI, RTL)
...
```
- **Token:** Stores token details, including its Chainlink price feed and ERC20 address (0x0 for RBTC).
...
constructor() Ownable(msg.sender) {
    ...
    tokenPrice[0] = RBTC_PRICE_USD;
    // Similar initialization for USDT, DAI, RTL
    tokenCount = 4;
}
```
- Initializes four tokens (RBTC, USDT, DAI, RTL) with their Chainlink price feeds and token prices.
...
function _calculateSwapAmount(
    ...
    if (_fromTokenId == 3) { // RTL as fromToken
        if (_toTokenId == 0) { // RTL to RBTC
            amountToReceive = (_amountToSwap * fromPrice) / 10**18;
        } else { // RTL to USDT/DAI
            uint256 ethValue = (_amountToSwap * fromPrice) / 10**18; // RTL in RBTC
            amountToReceive = (ethValue * RBTC_PRICE_USD) / toPrice; // RBTC to USD
        }
    } else if (_toTokenId == 3) { // USDT/DAI/RBTC to RTL
        // Similar logic for RBTC to RTL and USD to RTL
    } else { // RBTC/USDT/DAI to RBTC/USDT/DAI
...
- Handles special cases for RTL (priced in RBTC) versus other tokens (priced in USD).
...
function swapTokensForTokens(
    ...
    require(_fromTokenId != 0 && _toTokenId != 0, "Use RBTC swap functions for RBTC");
...
```
- Handles RBTC-to-token swaps by accepting `msg.value` and transferring tokens.
...
function addLiquidity(
    ...
    if (_token1Id != 0) {
        ...
        require(msg.value == _token1Amount, "Incorrect RBTC amount");
    }
    emit LiquidityAdded(...);
}
```
- Allows users to add liquidity by transferring tokens or RBTC.
...
### Prerequisites
- **Wallet:** A Web3 wallet (e.g., MetaMask) with RBTC and supported tokens.
...
1. **Connect Wallet:** Connect your wallet to the RootLend platform.
2. **Select Tokens:** Choose the `fromToken` and `toToken` (e.g., RBTC to USDT).
...
5. **Execute Swap:**
   - For RBTC-to-token: Call `swapEthForTokens` with RBTC.
   - For token-to-token: Call `swapTokensForTokens`.
   - For token-to-RBTC: Call `swapTokensForEth`.
6. **Confirm Transaction:** Sign the transaction in your wallet.

### Adding Liquidity
- Select a token pair (e.g., RBTC/USDT).
- Specify amounts for both tokens.
- Approve tokens (if not RBTC).
